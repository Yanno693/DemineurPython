...
def reinit():
    ...
    message.configure(text='Nombre de mines à trouver :')           # On remet ce message ici pour qu'il se réaffiche à chaque début de partie
    ...
    for i in range(lignes):                                         # Pour chaque ligne
        ...
        for j in range(colonnes):
            ...
            grille[i].append(dessin.create_rectangle(2+j*d, 2+i*d, 2+(j+1)*d, 2+(i+1)*d, outline='black', fill='gray'))

            if chance_mine <= 0.15 :                                # Si le nombre aléatoire est inférieur ou égal à 0.15, soit  15% de chance, 
                cellules[i].append(9)                               # Alors on a ajoute a la liste de cette case le numéro 9 ce qui signifie qu'elle a une bombe
                nb_mines = nb_mines + 1                             # Le nombre de mine au total sur le terrain augmente donc de 1
                nombre_mines.configure(text=nb_mines)               #...et l'on met à jour le texte affiché de nombre_mines à l'écran
                ...
                cellules[i].append(0)                               # Sinon on ajoute a la liste de cette case le numéro 0 ce qui signifie qu'elle n'a pas de bombe
...
def trace(i,j):
    ...
    verif = decouvertes.count(liste_rapide)                         # On crée une variable permettant de vérifier si les coordonnées se trouvent dans cette liste
    ...
    if verif == 0 and i >= 0 and i <= 14 and j >= 0 and j <= 19 and verif2 == 0:    # Si la case n'a pas été découverte, qu'elle n'a pas de drapeau et qu'elle se                          trouve bien dans la grille de jeu
        ...
        if cellules[i][j] == 0 :                                                    # On vérifie si la cellule ne comporte pas de mine
            ...
            if a == 0 :                                                             # Si il n'y a aucune mine au voisinage de celle-ci,
                ...
                dessin.create_text(2+d*(j+0.5), 2+(i+0.5)*d, fill='black', state=DISABLED, font= ('Courier', '10', 'bold'), text = str(a)) # On écrit le nombre de mines autour de la case sans affecter la fonction connexe    
        elif cellules[i][j] == 9:                                                                 #Si par contre la cellule porte le chiffre 9 dans sa liste, c'est à dire si la cellule est minée,
            dessin.itemconfigure(grille[i][j], fill='red')                                        #..alors on remplit la case de rouge
            dessin.create_oval(5+j*d, 5+i*d, (j+1)*d-1, (i+1)*d-1, outline='black', fill='black') #..et l'on y trace un disque noir symbolisant la mine
            ...
            for i in range (lignes) :                                                             # On révèle toutes les cases du jeu
                ...
                for j in range(colonnes) :   
                    ...
                    if cellules[i][j] == 0 :
                        ...
                        dessin.create_oval(5+j*d, 5+i*d, (j+1)*d-1, (i+1)*d-1, outline='black', fill='black')
                        liste_fin = [i,j]
                        decouvertes.append(liste_fin)
